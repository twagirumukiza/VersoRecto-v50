(function(){
  'use strict';
  const STORAGE_KEY='versoRecto.v50.experience';
  const LEGACY_KEY='versoRecto.v48.experience';
  const SESSION_KEY='versoRecto.v50.sessions';
  const MAX_POSITIONS=14000;
  const MIN_RELIABLE_PLAYS=3;
  let pending=[];

  function empty(){return {schema:'verso-recto-experience-v50',version:50,updatedAt:null,totalGames:0,totalMovesLearned:0,positions:{},profiles:{stable:null,candidate:null,championTests:[]}};}
  function migrateLegacy(){
    try{
      const old=JSON.parse(localStorage.getItem(LEGACY_KEY)||'null');
      if(!old||!old.positions)return null;
      return {schema:'verso-recto-experience-v50',version:50,updatedAt:old.updatedAt||null,totalGames:Number(old.totalGames||0),totalMovesLearned:Number(old.totalMovesLearned||0),positions:old.positions,profiles:{stable:null,candidate:null,championTests:[]}};
    }catch(e){return null;}
  }
  function load(){
    try{
  ...
  function beginGame(){pending=[];try{sessionStorage.removeItem('versoRecto.v50.pending');}catch(e){}}
  function recordMove(data){
    ...
    try{sessionStorage.setItem('versoRecto.v50.pending',JSON.stringify(pending));}catch(e){}
  }
  ...
  function finishGame(result){
    if(!pending.length){try{pending=JSON.parse(sessionStorage.getItem('versoRecto.v50.pending')||'[]')||[];}catch(e){pending=[];}}
    ...
    save(db);pending=[];try{sessionStorage.removeItem('versoRecto.v50.pending');}catch(e){}return true;
  }
  ...
  function exportData(){const db=load(),blob=new Blob([JSON.stringify(db,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='verso-recto-v50-experience-'+new Date().toISOString().slice(0,10)+'.json';document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove();},0);}
  function importData(file){return file.text().then(t=>{const x=JSON.parse(t);if(!x||!x.positions||![48,50].includes(x.version))throw new Error('Fichier V48/V50 invalide');if(x.version===48){x.version=50;x.schema='verso-recto-experience-v50';x.profiles={stable:null,candidate:null,championTests:[]};}save(x);return stats();});}
  function clear(){localStorage.removeItem(STORAGE_KEY);pending=[];try{sessionStorage.removeItem('versoRecto.v50.pending');}catch(e){}}
